...
rpcc::rpcc(sockaddr_in _dst, bool _debug)
  : dst(_dst), debug(_debug), xid(1), svr_nonce(0), bind_done(false), reachable (true), chan(_dst, _debug), destroy_wait (false)
{
...
rpcs::rpcs(unsigned int port, bool _debug)
  : debug (_debug), reachable(true), chan(port, _debug), nthread(0), deleting(false), counting(0) 
{
...
rpcs::dispatch(junk *j)
{
  ...
  if(!req.ok() || procs.count(proc) < 1){
    assert(pthread_mutex_unlock(&procs_m) == 0);
    if (debug) printf("bad proc %x req ok? %d procs.count %zu\n",
